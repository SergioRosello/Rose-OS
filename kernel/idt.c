...
//The PICs are initialized using 8-bit command words known as Initialization command words (ICW).
//In protected mode, the first command you will need to give the two PICs is the initialize command ICW1 (0x11).
...
//The second initialization command is the ICW2, written to the data ports of each PIC. 
//It sets the PIC’s offset value. 
//This is the value to which we add the input line number to form the Interrupt number.
//PICs allow cascading of their outputs to inputs between each other.
//This is setup using ICW3 and each bit represents cascading status for the corresponding IRQ.
